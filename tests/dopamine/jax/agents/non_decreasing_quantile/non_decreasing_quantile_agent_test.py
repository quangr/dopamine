...
from dopamine.jax.agents.non_decreasing_quantile import non_decreasing_quantile
...
class ImplicitQuantileAgentTest(absltest.TestCase):
  ...
  def _create_test_agent(self):
    ...
    class MockNonDecreasingQuantileNetwork(linen.Module):
      ...
      def __call__(self, x):
        ...
        state_net_tiled = jnp.tile(x, [self.num_quantiles+1, 1])
        x *= state_net_tiled
        ...
        quantiles = jnp.ones([self.num_quantiles, 1])
        return atari_lib.NonDecreasingQuantileNetworkType(quantile_values[0,:], quantile_values[1:,:],quantile_values[0,:])
    
    agent = non_decreasing_quantile.JaxNonDecreasingQuantileStarAgent(
        network=MockNonDecreasingQuantileNetwork,
  ...
  def testCreateAgentWithDefaults(self):
    ...
    agent = non_decreasing_quantile.JaxNonDecreasingQuantileStarAgent(num_actions=4)
